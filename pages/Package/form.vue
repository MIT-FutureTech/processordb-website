<template>
  ...
  <div class="min-h-screen max-w-7xl mx-auto py-2 px-4 justify-start md:justify-between items-center">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-8">
      ...
      >
        <span class="mr-2">Create Package</span>
        <svg
...
const handlePackageCreated = async (packageId) => {
  await navigateTo(`/package/${packageId}`);
};

const submitForm = () => {
  console.log('Submit form clicked');
  if (packageFormRef.value) {
    console.log('Calling createPackage method');
    packageFormRef.value.createPackage();
  } else {
