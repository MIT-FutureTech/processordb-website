  ...
  <div class="min-h-screen max-w-7xl mx-auto py-2 justify-start md:justify-between items-center">
    ...
    <div class="flex items-center justify-between mt-8 ml-2">
      <div class="flex items-center">
        <h1 class="text-4xl font-bold text-[#A32035]">{{ socId && socId !== 'null' ? 'Edit SoC' : 'Create SoC' }}</h1>
    ...
    <!-- SOC Form -->
    <div v-else-if="!pending || !socId || socId === 'null'" class="flex items-center justify-between mt-8 ml-2">
      <form class="w-full">
...
import { useRoute, useRuntimeConfig, useFetch, navigateTo } from '#imports'
...
// Initialize form with SoC data if editing
...
const submitForm = async () => {
  ...
  try {
    const config = useRuntimeConfig()
    const isEditing = socId && socId !== 'null'
    ...
    const url = isEditing ? `${config.public.backendUrl}/api/socs/${socId}` : `${config.public.backendUrl}/api/socs`
    ...
    if (response.ok) {
      alert(`SoC ${isEditing ? 'updated' : 'created'} successfully!`)
      navigateTo('/soc/list')
    } else {
      const errorData = await response.json()
      alert(`Failed to ${isEditing ? 'update' : 'create'} SoC data: ${errorData.error || 'Unknown error'}`)
    }
  } catch (error) {
    console.error('Error saving SoC:', error)
    alert('Error saving SoC data')
  }
}
