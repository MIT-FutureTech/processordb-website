...
export default defineEventHandler(async (event) => {
    ...
    try {
        const fpgaId = getRouterParam(event, 'id')
        
        if (!fpgaId) {
            throw createError({
                statusCode: 400,
                statusMessage: 'FPGA ID is required'
            })
        }
        ...
        if (forceRefresh) {
            ...
            console.log(`[FPGA API] Cache bypass requested for fpgaId=${fpgaId}`)
        } else {
        ...
        const url = `${backendUrl}${apiPrefix}/fpgas/${fpgaId}`
        ...
        console.log(`[FPGA API] FPGA ${fpgaId} fetched successfully`)
        return data
    } catch (error) {
        console.error('Error fetching FPGA:', error)
        throw createError({
            statusCode: error.statusCode || 500,
            statusMessage: error.statusMessage || 'Failed to fetch FPGA data'
        })
    }
})
