...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        if (forceRefresh) {
            ...
            console.log(`[GPU API] Cache bypass requested for page=${page}, limit=${limit}`)
        } else {
        ...
        const url = `${backendUrl}${apiPrefix}/gpus?page=${page}&pageSize=${limit}`
        ...
        console.log(`[GPU API] Backend response status: ${response.status}`)
        ...
        if (pagination) {
            console.log(`[GPU API] Pagination info: totalCount=${pagination.totalCount}, totalPages=${pagination.totalPages}`)
        }

        const totalTime = Date.now() - fetchStartTime
        console.log(`[PERF] GPU fetch timings - Fetch: ${fetchTime}ms, Parse: ${parseTime}ms, Total: ${totalTime}ms`)
