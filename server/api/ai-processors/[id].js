...
export default defineEventHandler(async (event) => {
    ...
    try {
        const aiProcessorId = getRouterParam(event, 'id')
        
        if (!aiProcessorId) {
            throw createError({
                statusCode: 400,
                statusMessage: 'AI Processor ID is required'
            })
        }
        ...
        if (forceRefresh) {
            ...
            console.log(`[AI Processor API] Cache bypass requested for aiProcessorId=${aiProcessorId}`)
        } else {
        ...
        const url = `${backendUrl}${apiPrefix}/ai-processors/${aiProcessorId}`
        console.log(`[AI Processor API] Fetching from backend: ${url}`)
        ...
        console.log(`[AI Processor API] AI Processor ${aiProcessorId} fetched successfully`)
        return data
    } catch (error) {
        console.error('Error fetching AI Processor:', error)
        ...
        throw createError({
            ...
            statusMessage: error.statusMessage || 'Failed to fetch AI Processor data'
        })
    }
})
